    ...
    @else
        ...
        <div class="obe-card p-0 overflow-hidden">
            <div class="px-3 py-3 border-bottom" style="background:var(--obe-bg);">
                ...
                <small class="text-muted">Minimal CPMK lulus: 70 · Nilai otomatis E jika ada CPMK gagal · <span class="fw-semibold">—</span> belum dinilai · <span class="fw-semibold">*</span> nilai sementara (belum lengkap)</small>
            </div>
                        ...
                        @foreach($rows as $i => $row)
                            <tr style="{{ $row['any_failed'] ? 'background:#fef2f2;' : '' }}">
                                ...
                                @foreach($row['cpmks'] as $cR)
                                    ...
                                    @endforeach
                                    @if($cR['total'] === null)
                                        <td class="text-center fw-bold text-muted" title="Belum dinilai">—</td>
                                    @else
                                ...
                                @php $incomplete = ! $row['complete'] && ! $row['any_failed']; @endphp
                                <td class="text-center fw-bold" style="color:{{ $row['any_failed'] ? 'var(--obe-red)' : 'inherit' }};">
                                    ...
                                    @else
                                        {{ number_format($row['final_score'], 2) }}@if($incomplete)<span class="text-muted">*</span>@endif
                                    @endif
                                </td>
                                <td class="text-center fw-bold">{{ $row['final_mutu'] !== null ? number_format($row['final_mutu'], 2) : '—' }}</td>
